...
object LazyList:
  ...
  def fibsRecurse(nMinus1: Int, n: Int): LazyList[Int] = LazyList.cons(n, fibsRecurse(n, nMinus1+n))
  def fibs: LazyList[Int] = fibsRecurse(0, 1)

  def unfold[A, S](state: S)(f: S => Option[(A, S)]): LazyList[A] = 
    f(state).fold(LazyList.empty)((a, s) => LazyList.cons(a, unfold(s)(f)))
