...
object List {
  ...
  def zipMap[A, B, C](f: (A, B) => C)(as1: List[A], as2: List[B]): List[C] = (as1, as2) match 
...
object exercise {
  @main def test =
    ...
    println(List.zipMap[Int, Int, Int]((x, y) => x * y)(l1, l2))
    println(List.zipMap[Int, Int, Int]((x, y) => x * y)(l1, l3))
}
