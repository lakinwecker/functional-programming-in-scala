...
object LazyList:
  ...
  def main =
    ...
    println(fibs.map(-_).take(13).toList)
    println(fibs.map(-_).drop(13).take(13).toList)
    println(fibs.map(-_).takeWhile(_ > -100).toList)
    println(fibs.take(13).zipWith(fibs.drop(13).take(14), (a: Int, b: Int) => a+b).toList)
    println(fibs.take(4).zipAll(fibs.take(9)).toList)
